...
type MutationsDictionary =
    ...
    with
        ...
        member x.getParameterForCustomPayload consumerResourceName (accessPathParts: AccessPath) =
            ...
            | Some entries when payloadType = CustomPayloadType.String ->
                    ...
                    | Some entry ->
                        ...
                        DictionaryPayload (payloadType, payloadName, isObject) |> stn
            | Some _ ->
                DictionaryPayload (payloadType, payloadName, false) |> stn
            | None -> Seq.empty
        ...
        member x.getParameterForCustomPayloadUuidSuffix consumerResourceName =
            ...
            | Some _ ->
                DictionaryPayload (payloadType, consumerResourceName, false) |> stn
            | None -> Seq.empty
