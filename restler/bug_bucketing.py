...
class BugBuckets(object):
    ...
    def update_bug_buckets(self, sequence, bug_code, origin='main_driver', reproduce=True, additional_log_str=None, lock=None):
        """ Update buckets of error-triggering test case buckets by potentially
        adding sequence.

        @param sequence: The sequence that triggers an error
        ...
        def get_create_once_requests(create_once_requests):
        ...
        def get_bucket_origin():
            """ Helper to get the bug bucket origin string """
        ...
        try:
            bucket_origin = get_bucket_origin()
            ...
            get_create_once_requests(create_once_requests)
            sent_request_data_list = create_once_requests + sent_request_data_list

            logger.update_bug_buckets(self._bug_buckets, sent_request_data_list, additional_log_str=additional_log_str)
        finally:
            if lock is not None:
                lock.release()
